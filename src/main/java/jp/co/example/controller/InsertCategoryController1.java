...
@Controller
@RequestMapping("/")
public class InsertCategoryController1 {

	@Autowired
	private InsertCategoryService1 insertCategoryService;
	...
	public String insertCategory() {
		...
		for (String bigCategoryName : bigCategoryList) {
			Category1 bigCategory = new Category1();
			...
			for (String middleCategoryName : middleNameList) {
				...
				Category1 middleCategory = new Category1();
				...
				for (String[] categoryNameArray : categoryNameArrayList) {
					...
					if (categoryNameArray[1].equals(middleCategoryName) && categoryNameArray[0].equals(bigCategoryName)) {
						//小カテゴリをインスタンス化
						Category1 smallCategory = new Category1();
						//必要情報をセット
